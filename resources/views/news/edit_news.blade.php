        ...
        <div class="container-fluid">
            <div class="row">
                ...
                <div class="col-lg-7" style="padding-bottom:120px">
                    <form action="admin/news/edit/{{$news->id_news}}" method="POST">
                        <input type="hidden" name="_token" value="{{@csrf_token()}}">
                        <div class="form-group">
                            <label>Content</label>
                            <input class="form-control" name="txtName" value="{{$news->content}}"
                                   placeholder="Please Enter TypeNews"/>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control custom-select" name="id_category" id="id_category">
                                @foreach($categories as $category)
                                    <option
                                            @if($news->id_cg == $category->id_cg)
                                            {{"selected"}}
                                            @endif

                                            value="{{$category->id_cg}}" name="txtCategory">{{$category->name}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            <label>Summary</label>
                            <textarea name="summary" id="summary" class="form-control" rows="2">{{$news->summary}}</textarea>
                        </div>
                        <div class="form-group">
                            <label>Description</label>
                            <textarea name="description" id="demo" class="form-control ckeditor" rows="5">{{$news->description}}</textarea>
                        </div>
